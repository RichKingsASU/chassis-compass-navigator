...
Deno.serve(async (req) => {
  try {
    ...
    if (!r.ok) {
      return new Response(JSON.stringify({ error: `BB /assets/data ${r.status}` }), { status: 502, headers: { "content-type": "application/json", ...cors } });
    }
    const data = await r.json() as any;
    if (!data.items?.length) {
      return new Response(JSON.stringify({ drained: true }), { headers: { "content-type": "application/json", ...cors } });
    }
    ...
    return new Response(JSON.stringify({ ok: true, ingested: data.items.length }), { headers: { "content-type": "application/json", ...cors } });
  } catch (e) {
