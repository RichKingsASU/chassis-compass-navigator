...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log("Extracting FLEXIVAN invoice from:", { pdf_path, excel_path, invoice_id });
      ...
      try {
        ...
        if (excelError) {
          ...
          for (let i = 1; i < jsonData.length; i++) {
            ...
            let amount = 0;
            const amountHeaders = ['Amount', 'Total', 'Charge', 'Amount Due', 'Invoice Amount', 'Total Amount', 'Outstanding Balance'];
    ...
    console.error("Error in extract-flexivan-invoice:", error);
    return new Response(
      JSON.stringify({
