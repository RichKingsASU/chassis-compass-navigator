-- Update validation to ensure days mismatch prevents 100% confidence
  ...
  LOOP
    ...
    SELECT 
      ...
      jsonb_build_object(
        ...
        'confidence', CASE
          -- Perfect match requires ALL conditions to be true
          WHEN LOWER(TRIM(t.chassis_number)) = LOWER(TRIM(v_chassis))
           ...
           AND (v_grand_total IS NULL OR t.rated_amount IS NULL OR ABS(v_grand_total - t.rated_amount) <= 0.01)
