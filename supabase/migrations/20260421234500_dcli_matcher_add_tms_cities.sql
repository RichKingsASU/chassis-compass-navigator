-- ============================================================================
-- Add pickup_loc_city / drop_loc_city to the mg_tms fuzzy-match payload.
-- mg_tms columns verified via \d public.mg_tms on the local DB:
--   pickup_loc_city, drop_loc_city (not pickup_city / delivery_city as the
--   original brief claimed).
-- ============================================================================
      ...
      UPDATE public.dcli_invoice_line_item
         SET match_type='tms', match_confidence=0.6,
             tms_match=jsonb_build_object(
               ...
               'pickup_loc_city',   v_tms.pickup_loc_city,
               'drop_loc_city',     v_tms.drop_loc_city),
             matched_at=now(), updated_at=now()
       WHERE id = v_line.id;
      v_fuzzy := v_fuzzy + 1;
      CONTINUE;
