-- ============================================================================
-- Fix match_dcli_line_items to match the actual dcli_activity schema on the
-- local DB (no invoice_number column; date_out/date_in are text, not
-- timestamptz). Drops the non-existent invoice_number reference and casts
-- text dates through NULLIF(...,'')::date like the rest of the DCLI code.
--
-- validate_dcli_line_items is unchanged and not replaced here.
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- dcli_activity: no invoice_number; date_out/date_in are text
    SELECT id,
           chassis,
           container,
