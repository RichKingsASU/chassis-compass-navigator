-- Update validate_dcli_invoice to include date validation, charges validation, and days validation
...
BEGIN
  ...
  LOOP
    ...
    SELECT jsonb_build_object(
      ...
      'rated_quantity', t.rated_quantity,
      'confidence', CASE
        -- Start with base equipment match confidence
      ...
      - CASE 
          ...
          ELSE 0 
        END
      -- Apply penalty for charges mismatch
      - CASE 
          ...
          ELSE 0 
        END
      -- Apply penalty for days mismatch  
      - CASE 
          WHEN v_tier_1_days IS NOT NULL AND t.rated_quantity IS NOT NULL 
               AND v_tier_1_days != t.rated_quantity 
          THEN 10 
          ELSE 0 
        END,
      'match_reasons', ARRAY[
        ...
        CASE WHEN v_tier_1_days IS NOT NULL AND t.rated_quantity IS NOT NULL 
                  AND v_tier_1_days = t.rated_quantity 
