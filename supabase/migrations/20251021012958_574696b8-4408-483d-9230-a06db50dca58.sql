-- Fix validate_dcli_invoice to use correct mg_tms column names
...
BEGIN
  ...
  LOOP
    ...
    SELECT 
      (t.quantity)::numeric as tms_days,
      (t.cust_rate_charge)::numeric as tms_amount,
      jsonb_build_object(
        'ld_num', t.ld_num,
        'so_num', t.so_num,
        ...
        'cust_rate_charge', t.cust_rate_charge,
        'quantity', t.quantity,
        'confidence', CASE
          ...
          WHEN LOWER(TRIM(t.chassis_number)) = LOWER(TRIM(v_chassis))
           ...
           AND (v_invoice_billable_days IS NULL OR t.quantity IS NULL 
                OR v_invoice_billable_days = (t.quantity)::numeric)
          THEN 100
          ...
          ELSE
            ...
            - CASE 
                WHEN v_invoice_billable_days IS NOT NULL 
                     ...
                     AND v_invoice_billable_days != (t.quantity)::numeric
                THEN 20 
                ELSE 0 
              END
            -- Apply smaller penalty for charges mismatch (informational only)
            - CASE 
                WHEN v_grand_total IS NOT NULL AND t.cust_rate_charge IS NOT NULL 
                     AND ABS(v_grand_total - (t.cust_rate_charge)::numeric) > 0.01 
                THEN 5
                ELSE 0 
              END
        END,
        'match_reasons', ARRAY[
          ...
          CASE WHEN v_grand_total IS NOT NULL AND t.cust_rate_charge IS NOT NULL 
                    AND ABS(v_grand_total - (t.cust_rate_charge)::numeric) <= 0.01 
    ...
    INTO v_tms_days, v_tms_amount, v_tms_match
    FROM mg_tms t
    WHERE 
      (
