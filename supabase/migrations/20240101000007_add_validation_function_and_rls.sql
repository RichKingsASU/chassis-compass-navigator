...
-- Cross-validates DCLI, CCM, TRAC invoice line items against mg_tms and
-- portpro_tms, writing match results to validation_results.
-- ─────────────────────────────────────────
...
BEGIN
  ...
  -- ── DCLI: match line items by chassis number against TMS ──
  FOR v_inv IN
    SELECT
  ...
  LOOP
    ...
    -- Try exact chassis match in mg_tms
    SELECT mt.cust_rate_charge INTO v_tms_amount
    FROM (
      SELECT COALESCE(
        (raw_data->>'cust_rate_charge')::numeric,
        0
      ) AS cust_rate_charge,
      chassis_number
      FROM mg_tms
    ...
    ELSE
      ...
      IF FOUND THEN
        ...
        v_tms_amount := 0; -- portpro_tms doesn't have amount
      END IF;
    END IF;
  ...
  -- ── CCM: match from ccm_invoice using row_data chassis field ──
  FOR v_inv IN
    SELECT
  ...
  LOOP
    ...
    v_variance := COALESCE(v_inv.invoice_amount, 0) - COALESCE(v_tms_amount, 0);
    v_status := CASE
      WHEN v_match_type = 'exact' THEN 'validated'
      ELSE 'unmatched'
    END;
  ...
  -- ── TRAC: match from trac_invoice_data using row_data chassis field ──
  FOR v_inv IN
    SELECT
  ...
  -- ── WCCP: match from wccp_invoice_data ──
  FOR v_inv IN
    SELECT
...
-- Replace permissive "allow all" with authenticated-only access.
...
BEGIN
  FOR tbl IN
    SELECT unnest(ARRAY[
      'ccm_invoice', 'ccm_invoice_data', 'ccm_activity',
      'dcli_invoice', 'dcli_invoice_line_item', 'dcli_invoice_staging',
      'trac_invoice', 'trac_invoice_data', 'trac_invoice_staging',
      'mg_tms',
      'portpro_tms',
      'invoices', 'invoice_lines',
      'gps_uploads', 'gps_data',
      'activity_log',
      'validation_results',
      'ytd_loads',
      'yard_events_data',
  ...
  LOOP
    ...
    EXECUTE format(
      'CREATE POLICY "Authenticated write" ON %I FOR INSERT TO authenticated WITH CHECK (true);',
      tbl
    );
    EXECUTE format(
      'CREATE POLICY "Authenticated update" ON %I FOR UPDATE TO authenticated USING (true) WITH CHECK (true);',
      tbl
    );
    EXECUTE format(
      'CREATE POLICY "Authenticated delete" ON %I FOR DELETE TO authenticated USING (true);',
      tbl
    );
  END LOOP;
END;
...
-- Keep anon read on chassis demo tables (needed for public demo)
-- These already have "public read" policies from migration 3, keep them.
-- Also allow anon read on app_settings.
DO $$
DECLARE
  tbl TEXT;
BEGIN
  FOR tbl IN
    SELECT unnest(ARRAY[
      'chassis', 'loads', 'monthly_stats', 'mr_events', 'idle_gaps'
    ])
  LOOP
    -- Add authenticated write policies (read already has public policy)
    EXECUTE format(
      'CREATE POLICY IF NOT EXISTS "Authenticated write" ON %I FOR INSERT TO authenticated WITH CHECK (true);',
      tbl
    );
    EXECUTE format(
      'CREATE POLICY IF NOT EXISTS "Authenticated update" ON %I FOR UPDATE TO authenticated USING (true) WITH CHECK (true);',
      tbl
    );
    EXECUTE format(
      'CREATE POLICY IF NOT EXISTS "Authenticated delete" ON %I FOR DELETE TO authenticated USING (true);',
      tbl
    );
  END LOOP;
END;
