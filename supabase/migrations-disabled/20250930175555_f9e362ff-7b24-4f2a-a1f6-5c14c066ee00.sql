-- Drop and recreate the validate_dcli_invoice function with enhanced TMS matching
...
BEGIN
  ...
  LOOP
    ...
    -- A single invoice line can match multiple TMS records (multiple LD/SO numbers)
    SELECT jsonb_agg(
      jsonb_build_object(
        'ld_num', t.ld_num,
        'so_num', t.so_num,
        ...
        'confidence', CASE
          ...
          WHEN LOWER(TRIM(t.chassis_number)) = LOWER(TRIM(v_line_item->>'chassis'))
           AND LOWER(TRIM(t.container_number)) = LOWER(TRIM(COALESCE(v_line_item->>'on_hire_container', v_line_item->>'container_out')))
          THEN 100
          -- High confidence: chassis matches and container is close
          WHEN LOWER(TRIM(t.chassis_number)) = LOWER(TRIM(v_line_item->>'chassis'))
          THEN 80
          -- Medium confidence: container matches but chassis differs
          WHEN LOWER(TRIM(t.container_number)) = LOWER(TRIM(COALESCE(v_line_item->>'on_hire_container', v_line_item->>'container_out')))
          THEN 60
          -- Low confidence: partial matches
          ELSE 40
        END,
        'match_reasons', ARRAY[
          CASE WHEN LOWER(TRIM(t.chassis_number)) = LOWER(TRIM(v_line_item->>'chassis')) 
            THEN 'Chassis match' ELSE NULL END,
          CASE WHEN LOWER(TRIM(t.container_number)) = LOWER(TRIM(COALESCE(v_line_item->>'on_hire_container', v_line_item->>'container_out'))) 
    ...
    WHERE 
      ...
      (
        LOWER(TRIM(t.chassis_number)) = LOWER(TRIM(v_line_item->>'chassis'))
        OR LOWER(TRIM(t.container_number)) = LOWER(TRIM(COALESCE(v_line_item->>'on_hire_container', v_line_item->>'container_out')))
      )
    ...
    v_rows := v_rows || jsonb_build_object(
      ...
      'chassis', v_line_item->>'chassis',
      'container', COALESCE(v_line_item->>'on_hire_container', v_line_item->>'container_out'),
