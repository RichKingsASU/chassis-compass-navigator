"""
BlackBerry Radar GPS Data Ingestion — MISSING-ONLY (Gap Fill) Mode
====================================================================
Identical to blackberry_radar_ingest.py with three modifications:

  1. MISSING_ONLY = True flag (top of file)
  2. phase1() inspects {table}_distance and only pulls chassis with
     fewer than 12 chunks already loaded
  3. main() skips phase2() entirely when MISSING_ONLY is True

Use this to fill gaps left by an interrupted full ingest without
re-pulling data you already have. To do a full run, flip MISSING_ONLY
to False (or just use the original blackberry_radar_ingest.py).

Run from PowerShell:
    python missing_ingest.py
"""
...
# Gap-fill mode: when True, only pull distance for chassis missing chunks.
# Phase 2 is also skipped. Set False to run a full ingest (or use the
# original blackberry_radar_ingest.py).
MISSING_ONLY = True
...
# with a small buffer so we don't race the clock during long runs.
HISTORY_START_MS  = int((datetime.now(tz=timezone.utc) - timedelta(days=730)).timestamp() * 1000) + (2 * 24 * 60 * 60 * 1000)  # 2yr - 2day buffer
...
def phase1(account_name, config, conn, auth):
    ...
    if repair_found: print(f"  {sorted(repair_found, reverse=True)}")
    ...
    # ──── Gap detection: which chassis are missing distance chunks? ────
    if MISSING_ONLY:
        with conn.cursor() as cur:
            cur.execute(f"""
        ...
        missing = [c for c in repair_found
                   if already_loaded.get(c, 0) < expected_chunks]
        complete = [c for c in repair_found
                    if already_loaded.get(c, 0) >= expected_chunks]
...
def main():
    print("BlackBerry Radar GPS Ingestion — MISSING-ONLY MODE" if MISSING_ONLY
          else "BlackBerry Radar GPS Ingestion")
    ...
    for name, config in ACCOUNTS.items():
        ...
        try:
            ...
            if not MISSING_ONLY:
                ...
                print(f"\n── Phase 2 SKIPPED (MISSING_ONLY mode) ──")
            log_sync_complete(
                conn, sync_id, name,
