...
export default function TRACNewInvoice() {
  ...
  async function handleSubmit() {
    ...
    setError(null)
    try {
      const filePath = `trac/${Date.now()}_${file.name}`
      const { error: uploadError } = await supabase.storage.from('trac-invoices').upload(filePath, file)
      if (uploadError) throw uploadError

      const { error: invError } = await supabase.from('trac_invoice').insert({
        source_file: filePath,
        provider: 'TRAC',
        status: 'pending',
        line_count: parsedData.length,
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/vendors/trac')} className="text-muted-foreground hover:text-foreground text-sm">&larr; Back to TRAC</button>
      ...
      {step === 'upload' && (
        <Card>
          <CardHeader><CardTitle>Upload TRAC Invoice File</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <p className="text-muted-foreground">Upload an Excel (.xlsx) or PDF file containing the TRAC invoice data.</p>
              ...
              <Table>
                ...
                <TableBody>
                  {rows.slice(0, 20).map((row, i) => (
                    <TableRow key={i}>
                      {columns.map(col => <TableCell key={col} className="text-sm">{String((row as any)[col] ?? '')}</TableCell>)}
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
            </div>
            {parsedData.length > 20 && <p className="text-sm text-muted-foreground text-center">Showing 20 of {rows.length} rows</p>}
            <div className="flex gap-3 pt-4">
              <Button variant="outline" onClick={() => setStep('upload')}>Back</Button>
              <Button onClick={handleSubmit} disabled={uploading}>{uploading ? 'Submitting...' : 'Submit Invoice'}</Button>
            </div>
          </CardContent>
        </Card>
      )}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="p-6 bg-green-50 border border-green-200 rounded-lg text-center">
              <p className="text-green-800 text-lg font-medium">TRAC invoice successfully uploaded!</p>
              <p className="text-green-700 text-sm mt-1">{rows.length} rows saved for review.</p>
            </div>
            <div className="flex gap-3">
              <Button onClick={() => navigate('/vendors/trac')}>Back to TRAC</Button>
