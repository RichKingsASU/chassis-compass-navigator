      ...
      try {
        ...
        for (const [cn, records] of chassisMap) {
          const sorted = records.sort((a, b) => {
            const da = new Date(String(a.create_date ?? '')).getTime() || 0
            const db = new Date(String(b.create_date ?? '')).getTime() || 0
            return db - da
          })
          const latest = sorted[0]
          const lastDate = new Date(String(latest.delivery_actual_date ?? latest.pickup_actual_date ?? latest.create_date ?? ''))
          ...
          rows.push({
            ...
            last_load_date: latest.delivery_actual_date ?? latest.pickup_actual_date ?? latest.create_date,
            total_loads: records.length,
            total_revenue: totalRevenue,
            last_updated: latest.create_date,
          })
        }
