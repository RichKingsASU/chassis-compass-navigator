...
export default function ChassisUtilization() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setError(null)
      try {
        // Aggregate from mg_data directly using correct column names
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>{loading ? <Skeleton className="h-9 w-16" /> : <p className="text-3xl font-bold">{data.length}</p>}</CardContent>
        </Card>
        <Card>
