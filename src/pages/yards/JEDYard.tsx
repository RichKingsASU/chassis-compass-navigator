...
export default function JEDYard() {
    ...
    async function load() {
      ...
      try {
        const [recRes, fileRes] = await Promise.all([
          supabase.from('yard_jed').select('*').order('report_date', { ascending: false }).limit(200),
          supabase.from('yard_uploads').select('*').eq('yard', 'JED').order('created_at', { ascending: false }),
        ])
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setError(null)
    try {
      const filePath = `jed/${Date.now()}_${file.name}`
      const { error: uploadError } = await supabase.storage.from('yard-jed').upload(filePath, file)
      if (uploadError) throw uploadError
      const { error: dbError } = await supabase.from('yard_uploads').insert({
        yard: 'JED', file_name: file.name, file_path: filePath,
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold">JED Yard</h1>
      ...
      <Tabs defaultValue="data">
        ...
        <TabsContent value="data" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Reports Uploaded</CardTitle></CardHeader>
        ...
        <TabsContent value="upload" className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-muted-foreground">Upload a yard report file for JED yard.</p>
              {uploadSuccess && <div className="p-3 bg-green-50 border border-green-200 rounded text-green-800 text-sm">Report uploaded!</div>}
          ...
          <Card>
            ...
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>File Name</TableHead><TableHead>Upload Date</TableHead><TableHead>Records</TableHead><TableHead>Status</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {files.length === 0 ? (
