...
export default function TRACPage() {
  const [invoices, setInvoices] = useState<TracInvoice[]>([])
  const [activities, setActivities] = useState<TracActivity[]>([])
    ...
    async function loadData() {
      ...
      try {
        const [invRes, actRes] = await Promise.all([
          supabase.from('trac_invoice').select('*').order('created_at', { ascending: false }),
          supabase.from('trac_activity').select('*').order('created_at', { ascending: false }).limit(20),
        ])
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                <p><span className="font-medium">Billing Support:</span> billing@tracintermodal.com</p>
                <p><span className="font-medium">Phone:</span> 1-800-367-8722</p>
                <p><span className="font-medium">Provider Code:</span> TRAC</p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                  ...
                  <p className="text-sm text-yellow-700">Disputes must be filed within 30 days of invoice date.</p>
                </div>
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-md">
                  ...
                  <ul className="text-sm text-blue-700 list-disc list-inside mt-1 space-y-1">
                    <li><a href="#" className="underline">TRAC Rate Schedule</a></li>
                    <li><a href="#" className="underline">Equipment Agreement</a></li>
        ...
        <TabsContent value="invoices" className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3 items-center">
              ...
              <Link to="/trac/new-invoice"><Button size="sm">+ New Invoice</Button></Link>
            </div>
          </div>
                  ...
                  {filteredInvoices.length === 0
                    ...
                    : filteredInvoices.map(inv => (
                      <TableRow key={inv.id}>
                        ...
                        <TableCell>
                          <Link to={`/trac/invoice/${inv.id}`}><Button variant="outline" size="sm">View</Button></Link>
                        </TableCell>
                      </TableRow>
                    ))}
                </TableBody>
