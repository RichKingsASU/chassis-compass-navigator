...
import { useFlexivanActivity } from '@/features/flexivan/hooks/useFlexivanActivity'
import { useVendorKpis } from '@/hooks/useVendorKpis'
import type { FlexivanActivityRow } from '@/features/flexivan/types'
import { 
  Building2, 
  ExternalLink, 
  ShieldAlert, 
...
const ACTIVITY_TABLE = 'flexivan_activity'
...
export default function FLEXIVANPage() {
  ...
  const { data: activity = [], isLoading: activityLoading } = useFlexivanActivity()

  const handleInvoicesLoaded = useCallback((rows: VendorInvoice[]) => {
    setInvoices(rows)
  }, [])

  const activityColumnDefs = useMemo<ColDef<FlexivanActivityRow>[]>(() => [
    { 
    ...
    <div className="p-8 space-y-8">
      <div className="flex items-center gap-4">
        <div className="p-4 bg-primary rounded-3xl text-primary-foreground shadow-2xl shadow-primary/20">
          <Shuffle size={32} strokeWidth={3} />
        </div>
        <div>
          <h1 className="text-4xl font-black tracking-tighter uppercase">FLEXIVAN HUB</h1>
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground">FlexiVan Intermodal Intelligence</p>
        </div>
      </div>
            ...
            <Card className="border-none shadow-xl bg-card/50 backdrop-blur-sm">
              ...
              <CardContent className="pt-6 space-y-6">
                <div className="space-y-1">
                  ...
                  <p className="text-xl font-black">FlexiVan</p>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Operational Portal</p>
                  <a href="https://flexivan.com" className="text-xl font-black text-primary hover:underline flex items-center gap-2" target="_blank" rel="noreferrer">
                    flexivan.com
            ...
            <Card className="border-none shadow-xl bg-amber-500/[0.03]">
              ...
              <CardContent className="pt-6">
                <p className="text-sm font-bold text-amber-800 leading-relaxed uppercase tracking-tight">
                  Disputes must be submitted within standard industry windows for intermodal equipment leasing.
                </p>
              </CardContent>
            </Card>
          </div>
