...
export default function FLEXIVANPage() {
  const [invoices, setInvoices] = useState<FlexivanInvoice[]>([])
  const [activities, setActivities] = useState<FlexivanActivity[]>([])
    ...
    async function loadData() {
      ...
      try {
        const [invRes, actRes] = await Promise.all([
          supabase.from('flexivan-invoices').select('*').order('created_at', { ascending: false }),
          supabase.from('flexivan_activity').select('*').order('created_at', { ascending: false }).limit(20),
        ])
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-2">
                <p><span className="font-medium">Company:</span> Flexi-Van Leasing (FLEXIVAN)</p>
                ...
                <p><span className="font-medium">Billing Support:</span> billing@flexivan.com</p>
                <p><span className="font-medium">Phone:</span> 1-800-FLEXI-VAN</p>
                <p><span className="font-medium">Provider Code:</span> FLEXIVAN</p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                  <p className="text-sm font-medium text-yellow-800">Billing Cycles</p>
                  <p className="text-sm text-yellow-700">Invoices are issued bi-weekly. Disputes within 30 days.</p>
                </div>
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-md">
                  ...
                  <ul className="text-sm text-blue-700 list-disc list-inside mt-1 space-y-1">
                    <li><a href="#" className="underline">Rate Schedule</a></li>
                    <li><a href="#" className="underline">Lease Agreement</a></li>
                    <li><a href="#" className="underline">Contact Directory</a></li>
                  </ul>
                </div>
              </CardContent>
            </Card>
        ...
        <TabsContent value="invoices" className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3 items-center">
              ...
              <Link to="/flexivan/new-invoice"><Button size="sm">+ New Invoice</Button></Link>
            </div>
          </div>
                  ...
                  {filteredInvoices.length === 0
                    ...
                    : filteredInvoices.map(inv => (
                      <TableRow key={inv.id}>
                        ...
                        <TableCell>
                          <Link to={`/flexivan/invoice/${inv.id}`}><Button variant="outline" size="sm">View</Button></Link>
                        </TableCell>
                      </TableRow>
                    ))}
                </TableBody>
