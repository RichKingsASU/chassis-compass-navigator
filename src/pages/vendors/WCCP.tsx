...
export default function WCCPPage() {
  const [invoices, setInvoices] = useState<WccpInvoice[]>([])
  const [activities, setActivities] = useState<WccpActivity[]>([])
    ...
    async function loadData() {
      ...
      try {
        const [invRes, actRes] = await Promise.all([
          supabase.from('wccp_invoice').select('*').order('created_at', { ascending: false }),
          supabase.from('wccp_activity').select('*').order('created_at', { ascending: false }).limit(20),
        ])
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                <p><span className="font-medium">Website:</span> <a href="https://www.wccp.com" className="text-primary underline" target="_blank" rel="noreferrer">www.wccp.com</a></p>
                <p><span className="font-medium">Billing Support:</span> billing@wccp.com</p>
                <p><span className="font-medium">Phone:</span> 1-800-WCCP-CHS</p>
                <p><span className="font-medium">Provider Code:</span> WCCP</p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                  <p className="text-sm font-medium text-yellow-800">West Coast Operations</p>
                  <p className="text-sm text-yellow-700">WCCP services POLA/POLB and Pacific Northwest terminals.</p>
                </div>
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-md">
                  ...
                  <ul className="text-sm text-blue-700 list-disc list-inside mt-1 space-y-1">
                    <li><a href="#" className="underline">WCCP Rate Schedule</a></li>
                    <li><a href="#" className="underline">Terminal Locations</a></li>
                    <li><a href="#" className="underline">Dispute Guide</a></li>
                  </ul>
                </div>
              </CardContent>
            </Card>
        ...
        <TabsContent value="invoices" className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3 items-center">
              ...
              <Link to="/wccp/new-invoice"><Button size="sm">+ New Invoice</Button></Link>
            </div>
          </div>
                  ...
                  {filteredInvoices.length === 0
                    ...
                    : filteredInvoices.map(inv => (
                      <TableRow key={inv.id}>
                        ...
                        <TableCell>
                          <Link to={`/wccp/invoice/${inv.id}`}><Button variant="outline" size="sm">View</Button></Link>
                        </TableCell>
                      </TableRow>
                    ))}
                </TableBody>
