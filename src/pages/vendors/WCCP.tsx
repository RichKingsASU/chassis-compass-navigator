...
import { useWccpActivity } from '@/features/wccp/hooks/useWccpActivity'
import { useVendorKpis } from '@/hooks/useVendorKpis'
import type { WccpActivityRow } from '@/features/wccp/types'
import { 
  Building2, 
  ExternalLink, 
  ShieldAlert, 
...
const ACTIVITY_TABLE = 'wccp_activity'
...
export default function WCCPPage() {
  ...
  const { data: activity = [], isLoading: activityLoading } = useWccpActivity()

  const handleInvoicesLoaded = useCallback((rows: VendorInvoice[]) => {
    setInvoices(rows)
  }, [])

  const activityColumnDefs = useMemo<ColDef<WccpActivityRow>[]>(() => [
    { 
  ...
  return (
    <div className="p-8 space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-4xl font-black tracking-tighter uppercase">WCCP HUB</h1>
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground">West Coast Chassis Pool</p>
        </div>
      </div>
            ...
            <Card className="border-none shadow-xl bg-card/50 backdrop-blur-sm">
              ...
              <CardContent className="pt-6 space-y-6">
                <div className="space-y-1">
                  ...
                  <p className="text-xl font-black">West Coast Chassis Pool</p>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Operational Portal</p>
                  <a href="https://wccp.com" className="text-xl font-black text-primary hover:underline flex items-center gap-2" target="_blank" rel="noreferrer">
                    wccp.com
            ...
            <Card className="border-none shadow-xl bg-amber-500/[0.03]">
              ...
              <CardContent className="pt-6">
                <p className="text-sm font-bold text-amber-800 leading-relaxed uppercase tracking-tight">
                  WCCP requires all billing adjustments to be processed through the central pool management system.
                </p>
              </CardContent>
            </Card>
          </div>
