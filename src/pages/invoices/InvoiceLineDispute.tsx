...
export default function InvoiceLineDispute() {
  ...
  async function handleCloseDispute() {
    ...
    try {
      const { error: updateErr } = await supabase.from('invoice_lines').update({
        status: 'pending', dispute_status: 'closed', closed_at: new Date().toISOString(),
      }).eq('id', lineId)
      if (updateErr) throw updateErr
      navigate(-1)
    } catch (err: unknown) {
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="p-6 bg-green-50 border border-green-200 rounded-lg text-center">
              ...
              <p className="text-green-700 text-sm mt-1">The dispute has been recorded and is open for review.</p>
            </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div><p className="text-muted-foreground">Invoice #</p><p className="font-mono">{line.invoice_number}</p></div>
          ...
          <Card>
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label>Reason</Label>
                  <Textarea value={disputeReason} onChange={e => setDisputeReason(e.target.value)} placeholder="Describe the dispute reason..." rows={3} required />
                </div>
                <div className="space-y-2">
                ...
                <div className="flex gap-3">
                  ...
                  {line.dispute_status === 'open' && (
                    <Button type="button" variant="outline" onClick={handleCloseDispute} disabled={submitting}>Close Dispute</Button>
                  )}
