...
export default function WCCPNewInvoice() {
  ...
  async function handleSubmit() {
    ...
    setError(null)
    try {
      const filePath = `wccp/${Date.now()}_${file.name}`
      const { error: uploadError } = await supabase.storage.from('wccp-invoices').upload(filePath, file)
      if (uploadError) throw uploadError
      const { error: invError } = await supabase.from('wccp_invoice').insert({
        source_file: filePath, provider: 'WCCP', status: 'pending',
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/vendors/wccp')} className="text-muted-foreground hover:text-foreground text-sm">&larr; Back to WCCP</button>
        <h1 className="text-3xl font-bold">New WCCP Invoice</h1>
      </div>
      {error && <div className="p-4 bg-destructive/10 text-destructive rounded-md">{error}</div>}
      {step === 'upload' && (
        <Card>
          <CardHeader><CardTitle>Upload WCCP Invoice File</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="p-6 bg-green-50 border border-green-200 rounded-lg text-center">
              <p className="text-green-800 font-medium">WCCP invoice uploaded successfully!</p>
            </div>
            <div className="flex gap-3">
              <Button onClick={() => navigate('/vendors/wccp')}>Back to WCCP</Button>
