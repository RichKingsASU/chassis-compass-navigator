// ── Shared types for DCLI invoice portal ─────────────────────────────────────
...
export const INVOICE_STATUSES = [
  ...
  'NEED TO EMAIL AM',
  'EMAILED AM',
  'NEED TO EMAIL CARRIER',
  'EMAILED CARRIER',
  'NEED TO EMAIL TERMINAL',
  'EMAILED TERMINAL',
] as const
...
export interface DcliInvoice {
  ...
  status: string | null
  portal_status: InvoiceStatus | null
...
export function eventLabel(e: InvoiceEvent): string {
  switch (e.event_type) {
    case 'status_change':
      return `Invoice status → "${e.to_status}"${e.from_status ? ` (was "${e.from_status}")` : ''}`
    case 'line_status_change':
      return `Line item status → "${e.to_status}"${e.from_status ? ` (was "${e.from_status}")` : ''}`
    case 'note_added':      return 'Note added'
    case 'document_uploaded':
      return `Document uploaded: ${(e.metadata as { filename?: string }).filename ?? ''}`
    case 'document_deleted':
      return `Document removed: ${(e.metadata as { filename?: string }).filename ?? ''}`
