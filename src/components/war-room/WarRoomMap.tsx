...
export function WarRoomMap({ data, onSelectChassis }: Props) {
  ...
  const layers = useMemo(() => [
    new ScatterplotLayer<WarRoomChassis>({
      ...
      getFillColor: (d) =>
        d.war_room_status
          ? STATUS_COLORS[d.war_room_status]
          : [148, 163, 184, 180],
  ...
  const getTooltip = useCallback((info: { object?: WarRoomChassis | null }) => {
    const object = info.object;
    if (!object) return null;
    return {
      html: `
        <div style="font-family:sans-serif;font-size:12px;line-height:1.5;max-width:220px">
          ...
          ${object.customer_name ? `<br/>Customer: ${object.customer_name}` : ''}
        </div>
      `,
      style: {
        background: 'white',
    ...
    <div className="relative w-full h-full">
      ...
      <div className="absolute bottom-6 left-4 bg-background/90 backdrop-blur-sm border border-border/50 rounded-lg p-3 text-xs space-y-1.5 shadow-sm">
        {([
          ...
          ['dormant_low',   'bg-amber-400', 'Dormant < 3d'],
          ['dormant_high',  'bg-red-500',   'Dormant 3d+'],
          ['in_transit',    'bg-blue-500',  'In Transit'],
          ['returned',      'bg-slate-400', 'Returned'],
