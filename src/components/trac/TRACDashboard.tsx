...
import TRACDetailView from './TRACDetailView';
import ImportantNotices from './dashboard/ImportantNotices';
import ContactsAndResources from './dashboard/ContactsAndResources';
import { useTRACData } from '@/hooks/useTRACData';

const TRACDashboard = () => {
  const { selectedRecord, setSelectedRecord } = useTRACData();
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">TRAC Control Center</h1>
      ...
      <Tabs defaultValue="dashboard" className="w-full">
        ...
        <TabsContent value="tracker" className="pt-4">
          <TRACInvoiceTracker onViewDetail={setSelectedRecord} />
        </TabsContent>
      </Tabs>
    </div>
  );
