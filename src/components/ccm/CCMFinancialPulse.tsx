...
const CCMFinancialPulse = () => {
  ...
  const dashboardMetrics = React.useMemo(() => {
    ...
    const overdue = invoices.filter(inv => {
      ...
      return inv.status !== 'paid' && daysDiff > 30;
    });

    const monthlyPaid = invoices.filter(inv => {
      const invoiceDate = new Date(inv.invoice_date);
      return inv.status === 'paid' && invoiceDate >= monthStart;
    });
    ...
    const totalOut = invoices.reduce((sum, inv) => sum + (parseFloat(String(inv.total_amount_usd)) || 0), 0);
    const overdueAmt = overdue.reduce((sum, inv) => sum + (parseFloat(String(inv.total_amount_usd)) || 0), 0);
    const pendingAmt = pending.reduce((sum, inv) => sum + (parseFloat(String(inv.total_amount_usd)) || 0), 0);
    const monthPaid = monthlyPaid.reduce((sum, inv) => sum + (parseFloat(String(inv.total_amount_usd)) || 0), 0);
                ...
                <div key={index} className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <div className={`w-3 h-3 rounded-full ${
                      ...
                      status.status === 'Disputed' ? 'bg-red-500' :
                      'bg-gray-500'
                    }`}></div>
...
export default CCMFinancialPulse;
